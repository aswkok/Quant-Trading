...
class YahooQuoteMonitor:
    ...
    def _fetch_initial_data(self):
        ...
        try:
            ...
            if self.interval_seconds < 60:  # Less than 60 seconds
    ...
    def _add_quote(self, timestamp, bid_price, ask_price, spread, spread_pct):
        """
        Add a quote to the dataframe using individual parameters.
        ...
        """
        quote_data = (timestamp, bid_price, ask_price, spread, spread_pct)
        self.add_quote_to_dataframe(quote_data)
    ...
    def get_macd_signal(self):
        ...
        # Check for crossover/crossunder - ensure we're handling boolean values properly
        crossover = False
        crossunder = False
        
        if 'crossover' in self.quotes_df.columns:
            # Convert to explicit boolean to avoid DataFrame truth value ambiguity
            crossover_val = latest['crossover']
            if isinstance(crossover_val, bool):
                crossover = crossover_val
            elif isinstance(crossover_val, (int, float)):
                crossover = bool(crossover_val)
            else:
                # Handle other types (like pandas Series) by checking if value is True-like
                try:
                    crossover = bool(crossover_val)
                except ValueError:
        ...
        if 'crossunder' in self.quotes_df.columns:
            ...
            if isinstance(crossunder_val, bool):
                crossunder = crossunder_val
            elif isinstance(crossunder_val, (int, float)):
